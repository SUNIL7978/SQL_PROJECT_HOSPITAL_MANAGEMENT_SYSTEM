...
INSERT INTO
PATIENT_DIAGNOSIS(Diagnosis,Prescription,Patient_ID,Physician_id)
VALUES
...
SELECT NAME FROM PHYSICIAN ORDER BY NAME ASC;

# Write a query in SQL to obtain the full name of the patients whose gender is male.

SELECT NAME,SURNAME,CONCAT(NAME," ",SURNAME) AS FULL_NAME,GENDER
FROM PATIENT
WHERE GENDER = "MALE";

# Write a query in SQL to find the name of the nurse who are the head of their department and are registered.

SELECT NAME,POSITION,REGISTERED
FROM NURSE
WHERE POSITION = "Head Nurse" AND REGISTERED = "YES";

# Write a query in SQL to find the name of the nurse who are Team Leader or not registered.

SELECT NAME,POSITION,REGISTERED
FROM NURSE
WHERE POSITION = "Head Nurse" AND REGISTERED = "No";

# Write a query to obtain the average cost of all the medical procedures.

SELECT ROUND(AVG(COST),1) AS AVG_COST 
FROM PROCEDURES;

# Write a query to obtain name and cost of the procedure whose cost is greater than 2000. 
...
# Write a query to drop phone column from patient table. 
...
# Write a query to find second maximum cost of medical procedure?

#1ST_WAY

WITH SECOND_HIGHEST_COST AS
(SELECT CODE,NAME,COST,
	DENSE_RANK() OVER(ORDER BY COST DESC) AS RANKING
FROM PROCEDURES)
SELECT * FROM SECOND_HIGHEST_COST WHERE RANKING =2;

# 2ND WAY 

SELECT MAX(COST) AS SECOND_HIGHEST_COST
FROM PROCEDURES
WHERE COST < (SELECT MAX(COST) FROM PROCEDURES);

# Write a query in SQL to obtain the name of the patients starting with letter A.

SELECT CONCAT(NAME," ",SURNAME) AS FULL_NAME,GENDER
FROM PATIENT 
WHERE CONCAT(NAME," ",SURNAME) LIKE 'A%';

# Write a query in SQL to obtain the name of the patients whose third letter is M.

SELECT *
FROM PATIENT 
WHERE SUBSTRING(NAME,3,1) = 'M';

SELECT CONCAT(NAME," ",SURNAME) AS FULL_NAME 
FROM PATIENT 
WHERE CONCAT(NAME," ",SURNAME) LIKE '__M%';

# Write a query in SQL to obtain the name of the patients whose name start with letter J and ends with Z.

SELECT CONCAT(NAME," ",SURNAME) AS FULL_NAME 
FROM PATIENT 
WHERE CONCAT(NAME," ",SURNAME) LIKE 'J%Z';

# Write a query to obtain patient details having patient id 11 to 20.
...
SELECT P.NAME AS DOCTORS_NAME,D.DEPT_NAME
FROM PHYSICIAN P
...
SELECT CONCAT(P.NAME," ",P.SURNAME) AS PATIENT_NAME,PH.NAME AS PHY_WHO_DID_PRELIMINARY_TRE
FROM PATIENT P 
LEFT JOIN PHYSICIAN PH 
ON P.PRIMARY_CHECK = PH.EMPLOYEEID;

# Write a query in SQL to obtain the name of the physician with the department who are done with affiliation.

SELECT 
	PH.NAME AS DOCTOR_NAME,
    D.DEPT_NAME
FROM PHYSICIAN PH 
INNER JOIN DEPARTMENT D 
ON PH.EMPLOYEEID = D.HEAD
INNER JOIN AFFILIATED_WITH A 
ON PH.EMPLOYEEID = A.PHYSICIANID
WHERE A.PRIMARYAFFILIATION = 'T';

# Write a query to obtain physician name, position and department they are affiliated with
...
#18.Write a query in SQL to obtain the patient name from which physician 
#they get primary checkup and also mention the patient diagnosis with prescription.
...
# Write a query in SQL to obtain the maximum cost of the medical procedure.

SELECT NAME,COST FROM PROCEDURES 
WHERE COST = (SELECT MAX(COST) FROM PROCEDURES);

# Write a query in SQL to obtain the details of patient who has diagnosed with chronic pain.

SELECT * FROM PATIENT 
WHERE PATIENT_ID IN (SELECT PATIENT_ID FROM PATIENT_DIAGNOSIS WHERE DIAGNOSIS = 'Chronic Pain');

# Write a query in SQL to obtain the procedure name and cost whose cost is greater than the average cost of all the procedure.

SELECT NAME,COST
FROM PROCEDURES
WHERE COST > (SELECT AVG(COST) FROM PROCEDURES);

#Write a query in SQL to obtain the procedure name and cost whose cost is less than the average cost of all the procedure.

SELECT NAME,COST
FROM PROCEDURES
WHERE COST < (SELECT AVG(COST) FROM PROCEDURES);

# Write a query in SQL to obtain the physician name who are either head chief or senior in their respective department.

SELECT * FROM PHYSICIAN
WHERE POSITION IN 
(SELECT POSITION 
	FROM PHYSICIAN 
		WHERE POSITION LIKE '%HEAD CHIEF%' OR POSITION LIKE '%SENIOR%');
        
# Write a query in SQL to obtain the employee id, physician name and position whose primary affiliation has not been done.

SELECT * FROM PHYSICIAN
WHERE EMPLOYEEID IN (SELECT PHYSICIANID FROM AFFILIATED_WITH WHERE PRIMARYAFFILIATION = 'f');
